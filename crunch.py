...
Options:
  ...
  --no-report, -n    Don't produce a report on exit
...
def main(args):
    ...
    if args['--no-report']:
        ...
        os.makedirs(os.path.join(out_path, 'heap'))
...
def create_report(exec_name, out_path):
    ...
    ptrs = [p for p in os.listdir(os.path.join(out_path, 'heap'))]
    heap = {}
    stats = []
    fatal = False
    fatal_msg = ''
